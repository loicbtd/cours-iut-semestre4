...
function transformer_xml_en_vecteur(xml_brut){
    let parseur = new DOMParser();
    let xml = parseur.parseFromString(xml_brut,"text/xml");

    let vecteur = Array();
    let vecteur_temporaire = Array();

    let clients = xml.getElementsByTagName("client");

    for (let i = 0; i < clients.length; i++) {
        vecteur_temporaire = Array();
        for (let j = 0; j < clients[i].childNodes.length; j++) {
            vecteur_temporaire.push(clients[i].childNodes[j].innerHTML);
        }
        vecteur.push(vecteur_temporaire);
    }
    console.log(vecteur);
    return vecteur;
}
...
function mettre_a_jour_donnees(){
    ...
    requete_client.onreadystatechange = function() {
        if (this.readyState === 4 && this.status === 200) {
            clients = transformer_xml_en_vecteur(this.responseText);
        }
    };
    ...
    requete_service.onreadystatechange = function() {
        if (this.readyState === 4 && this.status === 200) {
            services = transformer_xml_en_vecteur(this.responseText);
        }
    };
    ...
    requete_contient.onreadystatechange = function() {
        if (this.readyState === 4 && this.status === 200) {
            contient = transformer_xml_en_vecteur(this.responseText);
        }
    };
